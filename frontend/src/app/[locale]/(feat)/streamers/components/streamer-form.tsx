...
import { xiaohongshuDownloadConfig } from "@/src/lib/data/platform/xiaohongshu/definitions"
...
export function StreamerForm({ defaultValues, templateUsers, onSubmit }: StreamerConfigProps) {
	...
	const platformStreamerSchema = useMemo(() => {
		const schemaMap: Record<PlatformType, any> = {
			...
			[PlatformType.XIAOHONGSHU]: xiaohongshuDownloadConfig,
	...
	const handleSubmit = useCallback(
		async (data: StreamerSchema) => {
			try {
				...
				const shouldCloneAsTemplate =
					!!defaultValues?.id &&
					!defaultValues?.isTemplate &&
					!!submitData.isTemplate
				...
				if (shouldCloneAsTemplate) {
					const { id: _ignoredId, state: _ignoredState, ...templateCopy } = submitData
					...
					toastData(
						translations.toast.submitMessage,
						{
							...submitData,
							name: `${submitData.name}${submitData.name?.includes("(模板)") ? "" : " (模板)"}`,
						},
						"code"
					)
				} else {
