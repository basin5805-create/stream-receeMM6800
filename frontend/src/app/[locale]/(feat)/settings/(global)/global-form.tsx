...
export function GlobalForm({ appConfig, update, getEngineConfig, updateEngineConfig, strings }: GlobalFormProps) {
	...
	const onSubmit = useCallback(
		async (data: GlobalConfig) => {
			...
			const mergedConfig = { ...finalData }

			if (engineConfig?.type === "ffmpeg" || engineConfig?.type === "streamlink") {
				mergedConfig.useBuiltInSegmenter = engineConfig.useBuiltInSegmenter
				mergedConfig.exitDownloadOnError = engineConfig.exitDownloadOnError
			}

			if (engineConfig?.type === "kotlin") {
				mergedConfig.enableFlvFix = engineConfig.enableFlvFix
				mergedConfig.enableFlvDuplicateTagFiltering = engineConfig.enableFlvDuplicateTagFiltering
				mergedConfig.combineTsFiles = engineConfig.combineTsFiles
			}
			...
			try {
				...
				const updatePromises = Promise.all([
					...
					update({ ...mergedConfig, engineConfig: undefined }),
				])
